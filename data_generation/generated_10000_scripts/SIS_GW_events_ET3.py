...
RUN_NAME = os.environ.get("GW_RUN_NAME", "SIS_GW_events_ET3")
...
def generate_lensed_image(which_image, source_offset, seed_offset):
    ...
    tmp_data = os.path.join(tmp_dir, f"SIS_data_strain_{suffix}.tmp.npy")
    tmp_h = os.path.join(tmp_dir, f"SIS_h_strain_{suffix}.tmp.npy")
    tmp_t = os.path.join(tmp_dir, f"SIS_time_array_{suffix}.tmp.npy")
    tmp_snr_single = os.path.join(tmp_dir, f"SIS_optimal_SNR_single_{suffix}.tmp.npy")
    tmp_snr_net = os.path.join(tmp_dir, f"SIS_optimal_SNR_network_{suffix}.tmp.npy")
    tmp_snr_compat = os.path.join(tmp_dir, f"SIS_optimal_SNR_{suffix}.tmp.npy")
    ...
    final_data = f"SIS_data_strain_{suffix}.npy"
    final_h = f"SIS_h_strain_{suffix}.npy"
    final_t = f"SIS_time_array_{suffix}.npy"
    final_snr_single = f"SIS_optimal_SNR_single_{suffix}.npy"
    final_snr_net = f"SIS_optimal_SNR_network_{suffix}.npy"
    final_snr_compat = f"SIS_optimal_SNR_{suffix}.npy"
