...
BASE = Path("outputs")
OUT = Path("qc_showcase")
OUT.mkdir(exist_ok=True)

LENSED = {
    ("PM", "ET"): BASE / "PM_GW_events_ET",
    ("PM", "LIGO"): BASE / "PM_GW_events_LIGO",
    ("SIS", "ET"): BASE / "SIS_GW_events_ET",
    ("SIS", "LIGO"): BASE / "SIS_GW_events_LIGO",
}
UNLENSED = {
    "ET": BASE / "unlensed_GW_events_ET",
    "LIGO": BASE / "unlensed_GW_events_LIGO",
}
...
for col, detector in enumerate(["ET", "LIGO"]):
    ...
    for model in ["PM", "SIS"]:
        ...
        for key, vals in arrays.items():
            ...
            axh.hist(vals, bins=min(12, max(3, len(vals))), histtype="step", lw=1.4, label=label)
            step_cdf(axc, vals, label)
    for key, vals in unlensed_snr(detector).items():
        if detector == "LIGO" and key != "unlensed_network":
            continue
        vals = flatten_for_plot(vals)
        axh.hist(vals, bins=min(12, max(3, len(vals))), histtype="stepfilled", alpha=0.25, label=key)
...
for row, model in enumerate(["PM", "SIS"]):
    ...
    for ax, vals, title in zip(axes[row], [mu0, mu1, mut], ["mu_0", "abs(mu_1)", "mu_total"]):
        ax.hist(vals, bins=min(8, max(3, len(vals))), color="tab:blue", alpha=0.7)
...
for row, model in enumerate(["PM", "SIS"]):
    ...
    axes[row, 0].hist(dt, bins=min(8, max(3, len(dt))), color="tab:green", alpha=0.75)
...
for row, model in enumerate(["PM", "SIS"]):
    ...
    for ax, col in zip(axes[row], cols):
        ax.hist(params[col], bins=min(8, max(3, len(params))), color="tab:cyan", alpha=0.75)
