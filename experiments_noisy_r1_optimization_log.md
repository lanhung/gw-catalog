...
## 2026-05-29 可观测辅助参数重排实验

### 修改内容
- 新增 `scripts/experiments/21_observable_aux_reranker.py`。
- 使用 waveform ensemble 召回 top50，再加入真实场景可估计的辅助参数做二阶段重排：trigger time difference、sky angular separation、chirp mass/mass ratio/component mass differences、spin proxy、luminosity-distance ratio。
- 不使用 lens 真值、source id、pair id、`mu`、`t_d`、`z_l` 等泄漏字段。
- 额外测试 exact/mild/realistic/rough 参数估计误差扰动。

### 结果
| family | mode | R@1 | R@5 | R@10 | R@50 | 结论 |
|---|---|---:|---:|---:|---:|---|
| SIS | exact | 0.8600 | 0.8600 | 0.8600 | 0.8600 | top50 内几乎可完全重排 |
| PM | exact | 0.8103 | 0.8103 | 0.8103 | 0.8103 | top50 内几乎可完全重排 |
| SIS | realistic | 0.8527 | 0.8597 | 0.8600 | 0.8600 | 仍明显超过 0.7 |
| PM | realistic | 0.8100 | 0.8103 | 0.8103 | 0.8103 | 仍明显超过 0.7 |
| SIS | rough | 0.7993 | 0.8543 | 0.8580 | 0.8600 | 粗估计下仍超过 0.7 |
| PM | rough | 0.8083 | 0.8103 | 0.8103 | 0.8103 | 粗估计下仍超过 0.7 |

### 诊断
- waveform-only 最好 SIS R@1=0.4657、PM R@1=0.3683；加入可观测辅助参数后 SIS/PM 均超过 0.7。
- 单特征消融显示 sky/mass/distance exact 在模拟数据中几乎直接给出同源一致性，说明 exact 结果偏上界。
- 真实论文中应把 exact 写作 upper bound，并用 realistic/rough 扰动或 posterior overlap 作为更可信版本。

## 2026-05-29 最小辅助参数实验

目标：辅助参数尽量少，只测试 `delta_time`、`sky_sep` 以及少量组合。

关键结论：默认推荐 `delta_time + sky_sep` 两个辅助参数。realistic 扰动下，SIS R@1=0.8020，PM R@1=0.8073；rough 扰动下，SIS R@1=0.6370，PM R@1=0.8040。`mass_sky` 没有超过 `time_sky`，所以当前不把质量参数作为默认最小辅助输入。

完整文档：`docs/minimal_auxiliary_reranker_results_cn.md`。
完整结果：`runs/et10000_minimal_aux_selected/minimal_aux_selected_summary.csv`。

## 2026-05-29 Time-Sky 两辅助参数全量实验

固定只使用 `delta_time + sky_sep` 两个辅助参数，在完整 test split 上进行 catalog-level top-50 reranking。SIS realistic R@1=0.7997，PM realistic R@1=0.8087；SIS rough R@1=0.6320，PM rough R@1=0.8077。完整文档见 `docs/time_sky_aux_full_results_cn.md`，结果表见 `runs/et10000_time_sky_aux_full/time_sky_aux_full_summary.csv`。

## 2026-05-29 Delta Time + Gaussian Sky Map Overlap 实验

将 `delta_time + sky_sep` 替换为 `delta_time + log_sky_map_overlap`。当前数据没有真实 HEALPix skymap，因此使用基于 `ra/dec` 和 mode-level sigma 的二维高斯近似 overlap。结果：SIS realistic R@1=0.7887，PM realistic R@1=0.8080；相较 `sky_sep` 版本，SIS realistic 略低约 0.011，PM 基本不变。完整文档见 `docs/delta_time_skymap_overlap_results_cn.md`。
